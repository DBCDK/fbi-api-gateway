...
Access may be one of the following types:
...
- `RetrieverService`: Returns a document id. This id can be used to fetch the article via the [Retriever](#h1-retriever) service.
...
If a RetrieverService or InfomediaService entry is presented, the id can be used to fetch
the article through [Retriever](#h1-retriever) or [Infomedia](#h1-infomedia).
...
The _materialSelection_ field can contain publication status, selection group, and
librarian assessment information for a manifestation.

The _searchValues_ values can be used with the corresponding Complex Search indexes:
_term.cataloguedPublicationStatus_, _term.selectionGroup_, and _term.librarianAssessment_.

<InlineGraphiQL
  query={`query Example_MaterialSelection($pid: String!) {
  manifestation(pid: $pid) {
    titles {
      main
    }
