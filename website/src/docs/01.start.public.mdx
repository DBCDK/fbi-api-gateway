...
## Tracking user interactions (Pending Approval)

_This section is under review and may still change_
...
A unique identifier for the visitor. The method for generating this identifier depends on whether the user has given consent for tracking:
...
One of the traceIds from the response should be used as the x-parent-trace-id in subsequent requests to maintain continuity and link user actions effectively.
...
curl -X POST \
  ...
  -H "x-visitor-id: test-visitor-id" \
  -H "x-parent-trace-id: test-trace-id" \
  -d '{
