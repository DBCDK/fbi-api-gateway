...
Draft fields may also be present in the schema. They are marked separately, may change without the same stability guarantees, and are listed in the Draft Log for the selected token.
...
The draft log contains fields currently marked as draft in the schema for the selected token.

Draft fields, including all of their subfields, are available to use but may change or be removed without prior notice. Unlike established fields, changes to draft fields and their subfields are not subject to the standard three-month deprecation notice.
