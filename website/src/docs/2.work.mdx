...
## Reviews (new proposal, experimental, will change)

The _workReviews_ field returns a list of reviews for the work.

Note that:

- When _infomediaId_ is available, you can use the id to fetch the content of the review.
  Check out the [Infomedia section](#Infomedia).
- The field _urls_ is a list of online locations where the review is available.
- The field _librariansReview_ has content, when the review is created by a librarian (Materialevurdering).
- The field _periodica_ has content, when the origin of the review is published in a periodica.
