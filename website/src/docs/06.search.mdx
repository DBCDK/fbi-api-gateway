...
Some filters supports range. The following example uses ageRange. Other filters are lixRange and letRange.

<InlineGraphiQL
  query={`query Example_FilterRange($q: SearchQuery!, $facets: [FacetField!]!, $facetLimit: Int!, $filters: SearchFilters!) {
  search(q: $q, filters: $filters) {
    hitcount
    works(offset: 0, limit: 10) {
      titles {
...
<InlineGraphiQL
  query={`query Example_HoldingsFilters ($q: SearchQuery!, $offset: Int!, $limit: PaginationLimit!, $filters: SearchFilters!, $facets: [FacetField!]!) {
  search(q: $q, filters: $filters) {
    hitcount
    works(offset: $offset, limit: $limit) {
      titles {
