...
In a complex search response, results are returned as works. Each work can contain multiple manifestations (editions, formats, ISBNs, etc.). The `searchHits` field lists the manifestations within the work that matched the search query.
