...
<InlineGraphiQL
  ...
  variables={{ q: { all: "some query" }, offset: 0, limit: 1 }}
/>
...
<InlineGraphiQL
  query={`query ($q: SearchQuery!, $offset: Int!, $limit: PaginationLimit!, $holdingsFilters: HoldingsFilters!) {
  search(q: $q, holdingsFilters: $holdingsFilters) {
    hitcount
    works(offset: $offset, limit: $limit) {
      titles {
        main
  ...
  variables={{
    ...
    holdingsFilters: { branchId: ["775122"], status: ["OnShelf"] },
    offset: 0,
    limit: 5,
  }}
/>
